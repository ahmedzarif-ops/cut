...
description: The craft layer for UI polish, motion, and the invisible details that make software feel great. Encodes a design-engineering philosophy (from Emil Kowalski's work) for deciding whether to animate, which easing and duration to use, spring vs. duration, component behavior (buttons, popovers, tooltips, toasts, drawers), CSS transform and clip-path techniques, gesture/drag interactions, animation performance, and reduced-motion accessibility. Use whenever the work is making an interface feel right or reviewing UI/animation code — triggers include "make this feel polished", "review this animation", "which easing/duration", "the animation feels janky/sluggish", "should this animate", "why does this transition look off", "polish this component", "the button doesn't feel responsive", "add a drawer/toast/tooltip", and any before/after motion critique. Complements animation-vocabulary, review-animations, polish, and creative-director. Not for brand direction or copy.
---
...
> I'm ready to help you build interfaces that feel right. My knowledge comes from a design-engineering craft philosophy (grounded in Emil Kowalski's work on UI polish and motion — see [animations.dev](https://animations.dev/) to go deeper).
...
This matters when the browser is simultaneously loading content, running scripts, or painting. Shared Layout Animations on a page's tab bar can drop frames during page loads; switching to CSS animations (off the main thread) fixes it.
...
These principles come from building Sonner (a widely-adopted toast library) and apply to any component:
...
When items enter and exit a list (like a stacked drawer), the opacity change must work well with the height animation. This is often trial and error. There is no formula — you adjust until it feels right.
...
For touch interactions (drawers, swipe gestures), test on physical devices. Connect your phone via USB, visit your local dev server by IP address, and use Safari's remote devtools. A simulator is an alternative but real hardware is better for gesture testing.
