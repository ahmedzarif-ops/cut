...
- CUT now has a database-backed, source-linked nutrition catalog: 99 foods and
  89 meal templates, including 82 Bengali, Bangladeshi, Desi, or South Asian options. Every current catalog
...
- Provider-backed AI remains disabled. The owner accepted a $1 estimated
  model-cost ceiling per subscriber per calendar month and a $10 total provider
  ceiling for the first private beta. Activation still requires server-side
  enforcement of the per-user ceiling plus separate approval for a CUT-only
  credential, provider funding, and enabling production AI. The deterministic
  private-catalog fallback works without a provider. The cost-reviewed launch
  recommendation is `gpt-5.6-luna` with low reasoning, five text requests per
  subscriber per UTC day, and the existing $4.99 monthly/no-trial product; see
  `AI_PREMIUM_PRICING.md`. No provider spend or App Store price change is
  authorized by that recommendation.
- The current candidate passes the complete repository test command, the full
  TypeScript check, production API build, working App Store validator,
  non-billable production topology dry run, and `git diff --check`. Generated
  code matches the updated OpenAPI source; the tracked-drift check must be
  repeated after the candidate is committed because it intentionally compares
  generated bytes with Git `HEAD`. These results do not prove production
  deployment, TestFlight, App Review, or public-release acceptance.
...
- An Xcode **Sign to Run Locally** Release build from the current candidate
  completed successfully on September 4 and preserved the owner's existing
  signed-in Simulator session. Rendered tap-through verified the redesigned
  Home screen, top-right scan control, five-tab navigation, meal search,
  Bengali & Desi and protein/diet filters, compact unselected meal cards, and
  expanded serving/nutrition/log controls without a Clerk keychain error. The
  production API still returned its old 6-meal catalog (2 Bengali/Desi results),
  which is expected because production remains on the older commit. The new
  99-food/89-meal catalog is not live or TestFlight-verified.
