 ...
 * In the appended case the leftmost value is whatever the CLIENT sent, so
 * no position is trustworthy by itself: candidates are checked in order
 * (forwarded values, then Host) and the first one present in
 * `allowedHosts` wins — a spoofed value simply never matches. Exported so
 * that app.ts (clerkMiddleware callback) and this proxy middleware agree
 * on which hostname is canonical.
 */
export function getClerkProxyHost(
  ...
  for (const candidate of [...forwardedValues, req.headers.host]) {
